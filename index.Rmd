...
### Summary
Within the summary, the values were chosen in order to investigate a correlation with revenue (or funding) with the values, such as test scores, that would reflect quality of education. The statistics we looked at werethe total number of observations present in our data set,the highest total revenue spent on a single state, the highest total expenditure spent on a single state, the total expenditure of the state with the highest 8th grade reading score, and the state with the lowest attendance. 
```{r}
#summary
summary_info
```
...
We can see that from our 1715 observations, the largest total revenue is greater than the total expenditure, and that there seems to be funding being used in that state, and that the highest reading score in the 8th grade seems to be paired with a high expenditure of $16201905. The region with the lowest attendance would be the District of Columbia.
